...
# 加载MHA文件
     for filename in os.listdir(args.input):
        if filename.endswith('.mha'):
            # Read the .mha file
            image = sitk.ReadImage(os.path.join(args.input, filename))
            ...
            new_dir = os.path.join(args.output, 's' + filename[:-4])
            ...
            seg_image = sitk.ReadImage(os.path.join(args.input, 'segmentation', filename))
